...
function FindAction() {
    ...
    async function findProjects(categoryId = null) {
        ...
        if (categoryId) {
            url = `http://${apiUrl}/api/projects/by-category/${categoryId}/`;
        }
        await fetch(url, {
            method: 'GET',
            headers: {
    ...
    // Funkcja do wyszukiwania grantów
    async function findGrants() {
        await fetch(`http://${apiUrl}/api/grants/`, {
            method: 'GET',
            headers: {
                "Content-Type": "application/json",
                "Accept": "application/json",
    ...
    function handleCategoryChange(event) {
        ...
        // Wywołaj wyszukiwanie projektów na podstawie wybranej kategorii
        if (selectedCategoryId) {
            findProjects(selectedCategoryId);
        } else {
            // Jeśli nie ma wybranej kategorii, wyświetl wszystkie projekty
            findProjects();
        }
    }
            ...
            {!loading ? (
                <div className="container">
                    <div>
                        ...
                        {projects.length > 0 ? (
                            projects.map((project) => (
                                <div className="container pt-3" key={project.id}>
                                    <div className="row">
                    ...
                    <div>
                        ...
                        {grants.length > 0 ? (
                            grants.map((grant) => (
                                <div className="container pt-3" key={grant.id}>
                                    <div className="row">
                                        <div className="col-4 border rounded text-center">
